...
- `citations_missing`: did the answer have zero valid citations AND not match the abstention
  phrase? This is the previously-ambiguous case (see the Step 9 fix-pass finding) where a model
  either hallucinated an answer with no real source, or gave a correct grounded answer but
  simply forgot to include citation markers - a citation-format-compliance signal, not a
  correctness judgment. See citations/text_extractor.py for the exact detection logic and why
  a two-field design (over a single 3-way status) was chosen: the two conditions are genuinely
  independent checks (phrase match vs. citation presence), not mutually exclusive states, so a
  fixed enum of 3 combinations would be less expressive than two plain booleans.
"""
