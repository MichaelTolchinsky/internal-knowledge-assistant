...
## 4. Data Model (`src/knowledge_assistant/storage/models.py`)
...
Decided items are stated as plain decisions below. Only genuinely unresolved items remain open
here.

**Decided:**

- **Embedding model + vector dimension:** `sentence-transformers/all-MiniLM-L6-v2`, 384
  dimensions, cosine distance (HNSW index, `vector_cosine_ops`). Chosen for fast CPU inference
  and small footprint, appropriate for a local learning project. Changing it later requires an
  Alembic migration (vector column dimension) and re-embedding all existing chunks.
- **Chunking strategy:** character-based (not token-based), hand-rolled paragraph -> sentence
  -> word recursive splitting - no tokenizer dependency, keeps the pipeline simple/
  understandable per the project's learning goal. Defaults (`chunk_size=800`,
  `chunk_overlap=100`, in `.env.example`) are a starting point to be tuned via the evaluation
  experiment workflow, not a final answer. `RecursiveCharacterChunker` fails fast (`ValueError`)
  if `chunk_overlap >= chunk_size // 2`, so a misconfigured sweep can't silently blow up chunk
  count instead of erroring.
- **Current top-K / similarity threshold defaults:** `top_k=5`, `similarity_threshold` unset
  (no threshold). Revisable via the evaluation experiment workflow (section 3.3) like any other
  RAG variable - not treated as a final answer, just what the current baseline (section 12) ran
  with.
- **`EmbeddingModel.embed()` is async**, matching `LocalLLMClient.generate()`'s pattern: the
  blocking `sentence-transformers` `.encode()` call is offloaded internally via
  `asyncio.to_thread`, so the public signature is `async` and callers never need to think about
  threading themselves - calling a sync `embed()` directly from an `async def` FastAPI handler
  would otherwise block the event loop for the full inference duration.
...
- **Composition root:** `dependencies.py` holds a single shared `HuggingFaceEmbeddingModel`
  instance, reused for both query-time and ingestion-time embedding so document/query vectors
  stay comparable.
- **Dual LLM provider, `aioboto3` decision recorded ahead of Bedrock implementation.** This
  project originally scoped a single LLM provider (README Non-Goals: "multiple LLM providers"
  unless there's a concrete learning reason). A concrete reason emerged: local generation for
  cost-free development/testing, with real AWS Bedrock credits available for comparison later -
  a genuine opportunity to contrast a local vs. cloud-hosted LLM behind the same `LLMClient`
  Protocol. `LocalLLMClient` (transformers, `Qwen/Qwen2.5-0.5B-Instruct`, CPU, generation
  wrapped in `asyncio.to_thread` internally) and a minimal `get_llm_client()` factory that only
  supports `"local"` today (`llm_provider: Literal["local"]` - fails loudly via
  pydantic-settings validation if misconfigured, not silently) are implemented.
  **`BedrockLLMClient` is deferred**, not implemented yet.

  When it is implemented: **use `aioboto3`, not `boto3` + `asyncio.to_thread`** - a deliberate
  departure from the local-embedding/generation pattern above. `invoke_model` is I/O-bound
  (network round-trip), not CPU-bound like local embedding/generation, so the GIL-release
  argument that justifies `to_thread` for `sentence-transformers`/`transformers` doesn't apply
  here - a thread would sit idle waiting on a socket, consuming one of a bounded thread-pool
  slot (`min(32, cpu_count+4)`) for no computational reason. At this project's actual scale
  (single developer, low concurrency) that cost is real but functionally invisible either way;
  `aioboto3` was chosen consciously for learning value and prior familiarity, accepting its
  tradeoffs (community-maintained wrapper around botocore, can lag AWS's own release cadence,
  one more dependency surface) over `boto3`'s official-support advantage. Production systems
  commonly reach for `aioboto3`/async-native AWS clients once concurrent load makes thread-pool
  exhaustion an actual bottleneck - a genuine data point for when this recommendation would flip
  if it hadn't already been chosen.
- **`Answer.abstained` and `Answer.citations_missing` are two independent booleans, not one
  collapsed flag.** `abstained` means only "matched the documented abstention phrase family"
  (the model's explicit signal); `citations_missing` means "zero valid citations AND no phrase
  match" (a citation-format-compliance signal, not a correctness judgment). Kept as two plain
  booleans rather than a 3-way enum, since the two conditions are independent checks, not
  mutually exclusive states.
- **Evaluation dataset format and scoring method:** dataset is `evaluation/dataset/v1.jsonl` (57
...
- **Prompt template versioning:** `PromptBuilderV1.template_version` ("v1") is currently just a
  hardcoded label - no content hash, changelog linkage, or persisted mapping from a given eval
  run's results back to the exact prompt text/instructions used at that time. The evaluation
  runner will eventually need something that actually ties the version string to the exact
  template content rather than trusting a manually-bumped constant (e.g. a hash of
  `_INSTRUCTIONS` computed at import time, or a stricter review requirement that any wording
  change bumps the version).
- **Local model citation/abstention compliance gap:** `Qwen/Qwen2.5-0.5B-Instruct` frequently
  does not reliably follow the exact citation-marker format or abstention phrasing instructed in
  `prompts/v1.py`, confirmed via real end-to-end runs (correct, grounded answers with zero
  citation markers; on at least one no-context question it hallucinated a plausible-sounding
  answer instead of abstaining). The `/query` pipeline is otherwise genuinely working
  end-to-end - this is specifically a small-model instruction-following limitation, not a bug
  in the citation/abstention detection logic itself. Options: strengthen the prompt (e.g.
  few-shot citation examples), or compare against Bedrock once implemented - do not assume the
  local model's compliance rate
  generalizes.
- **No "change one variable, re-run, compare" experiment has been run yet** - only the one
  baseline in section 12 exists so far. The workflow in section 3.3 is implemented and usable
  (`python -m knowledge_assistant.evaluation.runner`), but no actual comparison run (different
  chunk size, top-K, prompt, or embedding model) has been executed and recorded against the
  baseline.

## 12. First Baseline Evaluation Run (local provider, `Qwen2.5-0.5B-Instruct`)
...
**Groundedness = 0% is a known, already-documented gap, not a new bug or a scoring error:** the
local model gives correct, well-retrieved answers (79.5% correctness, 100% recall) but almost
never emits the `(source: ..., chunk N)` marker the groundedness metric requires - so real
citations rarely materialize even though retrieval and generation are genuinely working. This is
exactly the citation-compliance gap `Answer.citations_missing` was built to surface honestly
rather than hide behind an inflated groundedness number - see section 11's "still open" items.
Revisit by strengthening the prompt (few-shot citation examples) or comparing against Bedrock
once implemented, before trusting groundedness as a real signal for this provider.

This is the baseline for the "change one variable, re-run, compare" workflow (section 3.3) -
